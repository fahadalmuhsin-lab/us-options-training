...
function toggleMenu(){const p=document.getElementById('mobilePanel');if(!p)return; const opening=!p.classList.contains('open');p.innerHTML=`<div class="mobile-menu-card"><div class="mobile-menu-head"><b>${t('menu')}</b><button class="btn icon" onclick="toggleMenu()">×</button></div><a href="#/">${t('home')}</a>${isLogged()?`<a href="#/contracts">${t('contracts')}</a>`:''}${state.role==='admin'?`<a href="#/admin/users">${t('users')}</a><a href="#/admin/permissions">${t('permissions')}</a><a href="#/admin/contact">${t('contactInbox')}</a>`:`<a href="#/contact">${t('contact')}</a>`}<a href="#/logout">${t('logout')}</a></div>`;p.classList.toggle('open',opening)}
...
function memberHome(){return `<div class="section-title"><h2>${t('home')}</h2><span class="badge green">${t('live')}</span></div><div class="stats">${stat(t('total'),'24')}${stat(t('entryTotal'),'$31.80')}${stat(t('loss'),'$4.25','red')}${stat(t('net'),'$18.40+','green')}</div><div class="section-title"><h2>${t('contracts')}</h2><button class="btn" onclick="go('#/contracts')">${t('contracts')}</button></div><div class="card table-wrap"><table class="table"><thead><tr><th>${t('contracts')}</th><th>${t('entry')}</th><th>${t('current')}</th><th>${t('status')}</th></tr></thead><tbody>${['CRM','NVDA','AAPL','AMZN','MSFT','TSLA','META','GOOGL'].map(s=>`<tr onclick="selectSymbol('${s}')" style="cursor:pointer"><td>${s}<div class="company">${symbols[s].company}</div></td><td>${symbols[s].entry}</td><td>${symbols[s].current}</td><td class="${symbols[s].type==='CALL'?'call':'put'}">${symbols[s].type}</td></tr>`).join('')}</tbody></table></div>`}
