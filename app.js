...
function favorites(){return `<div class="section-title"><h2>${t('favorites')}</h2></div><div class="stats">${state.fav.map(s=>`<div class="card fav-card" onclick="selectSymbol('${s}')"><div class="symbol">★ ${s}</div><div class="${symbols[s].type==='CALL'?'green':'red'} big-value">${symbols[s].type}</div><p class="company">${symbols[s].company}</p></div>`).join('')}</div>`}
