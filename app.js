...
if(localStorage.getItem('saqr_theme')==='light')document.documentElement.classList.add('light-theme');
let headerQuery='';
...
function header(){
 ...
 return `<header class="topbar">
  <div class="topbar-start">
   <button class="btn icon theme-btn" aria-label="theme" onclick="toggleTheme()">${isLight?'☀️':'🌙'}</button>
  ...
  <a class="topbar-brand" href="#/"><span class="brand-mark">🦅</span><span class="brand-text-group"><span class="brand-title">${t('brand')}</span><span class="brand-sub">SAQR OPTIONS</span></span></a>
  <div class="topbar-end">
   <button class="btn icon notif-btn" aria-label="alerts" onclick="goAlerts()">🔔<span class="notif-badge ${badge?'':'zero'}">${badge}</span></button>
   <button class="menu-btn" aria-label="${t('menu')}" onclick="toggleMenu()"><i></i><i></i><i></i></button>
  </div>
 </header>`
}
function toggleTheme(){document.documentElement.classList.toggle('light-theme');localStorage.setItem('saqr_theme',document.documentElement.classList.contains('light-theme')?'light':'dark');render()}
...
function sidebar(route){
 ...
 if(state.role==='admin'){
  ...
  items=[['home','/'],['contracts','/contracts'],['favorites','/favorites'],['analyses','/analyses'],['alerts','/alerts']];
...
function toggleMenu(){const p=document.getElementById('mobilePanel');if(!p)return;const opening=!p.classList.contains('open');p.classList.toggle('open',opening);if(!opening){p.innerHTML='';return}let links=`<a href="#/" onclick="closeMenu();go('#/');return false;">${t('home')}</a>`;if(isLogged())links+=`<a href="#/contracts" onclick="closeMenu()">${t('contracts')}</a>`;if(state.role==='member')links+=`<a href="#/analyses" onclick="closeMenu()">${t('analyses')}</a><a href="#/favorites" onclick="closeMenu()">${t('favorites')}</a><a href="#/alerts" onclick="closeMenu()">${t('alerts')}</a><a href="#/contact" onclick="closeMenu()">${t('contact')}</a>`;else if(state.role==='admin')links+=`<a href="#/admin/users" onclick="closeMenu()">${t('users')}</a><a href="#/admin/permissions" onclick="closeMenu()">${t('permissions')}</a><a href="#/admin/contact" onclick="closeMenu()">${t('contactInbox')}</a>`;else links+=`<a href="#/subscription" onclick="closeMenu()">${t('subscription')}</a><a href="#/login" onclick="closeMenu()">${t('login')}</a><a href="#/contact" onclick="closeMenu()">${t('contact')}</a>`;p.innerHTML=`<div class="mobile-menu-card"><div class="mobile-menu-head"><b>${t('menu')}</b><button type="button" class="btn icon" onclick="toggleMenu()">×</button></div>${links}<a href="#/logout" onclick="logout();return false;">${t('logout')}</a></div>`}
...
function memberHome(){
 const u=getUsers().find(x=>x.phone===DEMO_MEMBER_PHONE)||{plan:'بدون اشتراك',status:'none',expires:'—'};
 const d=symbols[state.symbol];
 const status=u.banned?t('banned'):u.status==='active'?t('active'):t('pending');
 return `<div class="section-title"><h2>${t('home')}</h2><span class="badge green">${t('member')}</span></div><div class="admin-grid"><div class="card stat"><div class="label">${t('plan')}</div><div class="num">${u.plan}</div></div><div class="card stat"><div class="label">${t('status')}</div><div class="num green">${status}</div></div><div class="card stat"><div class="label">${t('expires')}</div><div class="num">${u.expires}</div></div><div class="card stat"><div class="label">${t('current')}</div><div class="num green">${d.current}</div></div></div><div class="grid cols-2"><div class="card"><div class="option-head"><div><div class="symbol">★ ${state.symbol}</div><div class="company">${d.company}</div></div><span class="badge ${d.type==='PUT'?'red':'green'}">${d.type}</span></div><div class="price-grid"><div class="price current"><div>${t('current')}</div><div class="v red">${d.current}</div></div><div class="price entry"><div>${t('entry')}</div><div class="v green">${d.entry}</div></div></div><div class="meta-grid"><div class="meta"><span class="small">${t('remaining')}</span><b>2 ${t('days')}</b></div><div class="meta"><span class="small">${t('expiry')}</span><b>${d.expiry}</b></div></div></div><div class="card targets"><h2>${t('goals')} 🎯</h2>${d.targets.map((x,i)=>`<div class="target"><span>${state.lang==='ar'?'الهدف':'Target'} ${i+1}</span><b>${x[0]}</b><span class="pct">${x[1]}</span></div>`).join('')}</div></div>`
}
...
function favorites(){return `<div class="section-title"><h2>${t('favorites')}</h2></div><div class="stats">${state.fav.length?state.fav.map(s=>`<div class="card fav-card" onclick="selectSymbol('${s}')"><div class="symbol"><span class="fav-star inline on" onclick="event.stopPropagation();toggleFav('${s}')">★</span> ${s}</div><div class="${symbols[s].type==='CALL'?'green':'red'} big-value">${symbols[s].type}</div><p class="company">${symbols[s].company}</p></div>`).join(''):`<div class="card empty"><h2>${t('noData')}</h2></div>`}</div>`}
