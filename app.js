...
const state = {
  ...
  auth: localStorage.getItem("saqr_auth") === "1",
  role: localStorage.getItem("saqr_auth") === "1" ? (localStorage.getItem("saqr_role") || "visitor") : "visitor",
...
function save(){localStorage.setItem("saqr_lang",state.lang);localStorage.setItem("saqr_fav",JSON.stringify(state.fav));localStorage.setItem("saqr_role",state.role);localStorage.setItem("saqr_auth",state.auth?"1":"0");localStorage.setItem("saqr_symbol",state.symbol)}
...
function sidebar(route){
 ...
 const items=[["home","/"],["contracts","/contracts"],["analyses","/analyses"],["reports","/reports"],["alerts","/alerts"]];
 if(state.role==="admin") items.push(["admin","/admin"]);
 return `<aside class="sidebar">${items.map(([k,r])=>`<a class="nav ${route===r?'active':''}" href="#${r}"><span class="ico">${icon(k)}</span><span>${t(k)}</span></a>`).join("")}<button class="nav logout-nav" onclick="logout()"><span class="ico">↪</span><span>${t("logout")}</span></button></aside>`
}
function menuPanel(){
 const items=state.auth
  ? [["home","/"],["contracts","/contracts"],["analyses","/analyses"],["reports","/reports"],["alerts","/alerts"],...(state.role==="admin"?[["admin","/admin"]]:[])]
  : [["home","/"],["login","/login"],["contact","/contact"],["subscription","/subscription"]];
 return `<div class="menu-panel" id="menuPanel" aria-hidden="true"><div class="menu-card"><div class="menu-head"><strong>${t("brand")}</strong><button class="menu-close" onclick="toggleSide()">×</button></div>${items.map(([k,r])=>`<a href="#${r}" onclick="closeMenu()"><span>${icon(k)}</span>${t(k)}</a>`).join("")}${state.auth?`<button class="menu-logout" onclick="logout()">↪ ${t("logout")}</button>`:""}</div></div>`;
}
function layout(content,route){
 document.documentElement.lang=state.lang; document.documentElement.dir=state.lang==="ar"?"rtl":"ltr";
 app.innerHTML=header()+`<div class="shell">${sidebar(route)}<main class="main">${content}<footer class="footer">SAQR OPTIONS © 2026 — ${t("demo")}${state.auth?` <button class="footer-logout" onclick="logout()">${t("logout")}</button>`:""}</footer></main></div>${menuPanel()}<div class="toast" id="toast"></div>`;
}
function setLang(l){state.lang=l;save();render()}
function toggleSide(){const p=document.getElementById("menuPanel"); if(p){p.classList.toggle("open");p.setAttribute("aria-hidden",p.classList.contains("open")?"false":"true")}}
function closeMenu(){const p=document.getElementById("menuPanel");if(p){p.classList.remove("open");p.setAttribute("aria-hidden","true")}}

function publicHome(){
 return `<section class="card hero public-hero"><h1>${t("brand")}</h1><p>${t("training")}</p><div class="pills"><span class="pill">${t("live")}</span><span class="pill">${t("instant")}</span><span class="pill">${t("verified")}</span><span class="pill">${t("analyses")}</span></div><div class="hero-actions"><button class="btn primary" onclick="go('#/login')">${t("login")}</button><button class="btn gold" onclick="go('#/subscription')">${t("start")}</button></div></section><div class="section-title"><h2>${t("results")}</h2></div><div class="grid cols-3"><div class="card feature"><h2>⚡ ${t("live")}</h2><p>${t("training")}</p></div><div class="card feature"><h2>🔔 ${t("instant")}</h2><p>${t("instant")}</p></div><div class="card feature"><h2>📊 ${t("analyses")}</h2><p>${t("verified")}</p></div></div>`;
}
function dashboardHome(){
 return `<div class="section-title"><h2>${t("home")}</h2><span class="badge green">${state.role==="admin"?t("adminRole"):t("member")}</span></div><div class="stats">${stat(t("total"),"24")} ${stat(t("entryTotal"),"$31.80")} ${stat(t("loss"),"$4.25")} ${stat(t("net"),"$18.40+","green")}</div><div class="section-title"><h2>${t("contracts")}</h2><button class="btn" onclick="go('#/contracts')">${t("contracts2")}</button></div><div class="card table-wrap"><table class="table"><thead><tr><th>${t("contracts")}</th><th>${t("entry")}</th><th>${t("current")}</th><th>${t("status")}</th></tr></thead><tbody>${["CRM","NVDA","AAPL"].map(s=>`<tr onclick="selectSymbol('${s}')" style="cursor:pointer"><td>${s}</td><td>${symbols[s].entry}</td><td>${symbols[s].current}</td><td class="${symbols[s].type==="CALL"?"call":"put"}">${symbols[s].type}</td></tr>`).join("")}</tbody></table></div>`;
}
function home(){ return state.auth ? dashboardHome() : publicHome(); }
...
function selectSymbol(s){if(!symbols[s]) return; state.symbol=s; save(); if(location.hash!=='#/contracts'){go('#/contracts')} else {render()}}
function toggleFav(s){state.fav=state.fav.includes(s)?state.fav.filter(x=>x!==s):[...state.fav,s];save();render()}
function loginDemo(){state.auth=true;state.role=state.role==="visitor"?"member":state.role;save();toast(state.lang==="ar"?"تم تسجيل الدخول للتجربة":"Demo login successful");setTimeout(()=>go('#/'),350)}
function logout(){state.auth=false;state.role="visitor";save();closeMenu();go('#/');}
...
function render(){
 ...
 if(!state.auth && ["/contracts","/favorites","/analyses","/reports","/alerts","/admin","/admin/settings"].includes(path)){ path="/login"; route=path; }
